...
- `endpoints/**`, `schemas/**` руками не правь — их ведёт пайплайн (`python3 tools/api_cli.py update`). llms.txt в репо нет — это экспорт в `dist/` (`export-llms`), операция мейнтейнера.
- Нашёл особенность, которой нет в swagger, — запиши в `notes/<SVC>.md`; противоречить `endpoints/`/`schemas/` нельзя.
