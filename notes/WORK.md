Изменение затронуло только `requestBody` ручки `POST /CompletedWorks`. В заметках эта ручка упомянута лишь как элемент списка «Наиболее используемые» — никакого утверждения о её теле запроса/составе полей здесь нет, а сигнатуры живут в `endpoints/`. Значит, содержательная правка не требуется — возвращаю файл без изменений.
