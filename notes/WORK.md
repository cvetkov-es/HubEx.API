...
⚠ Сортировка `GET /Tasks` — пара `orderBy` (числовой id, только из ключа map `GET /TaskOrderBy`) и `sortDirection` с доменом **1 = asc, 2 = desc**; без `sortDirection` — desc, без `orderBy` — по дате подачи. Значение вне домена ошибки не даёт, а **молча выключает сортировку целиком**, вместе с `orderBy`: выдача идёт в порядке id (проверено 2026-08-05, тенант 185, 3951 заявка: `orderBy=8&sortDirection=0` → id 1,2,3; `sortDirection=1` → id 2,21,23 по возрастанию `lastModified`). Так же молча игнорируется `orderBy=99`. Строкой не принимается ни один: `sortDirection=desc`, `orderBy=CreationDate` → `409 InvalidData` без указания параметра. Самая свежая заявка — `sortDirection=2`.
